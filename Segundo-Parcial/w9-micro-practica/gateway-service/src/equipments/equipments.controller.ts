...
export class EquipmentsController {
  ...
  constructor(
    @Inject('EQUIPMENTS_SERVICE') private readonly equipmentsClient: ClientProxy,
  ...
  async findAll(): Promise<ServiceResponse<Equipment[]>> {
    this.logger.log('📚 GET /api/equipments → equipments-service [equipment.find.all]');
    
    try {
      const response = await firstValueFrom(
        this.equipmentsClient.send<ServiceResponse<Equipment[]>>('equipment.find.all', {}).pipe(
          timeout(5000),
          catchError(() => {
  ...
  async findAvailable(): Promise<ServiceResponse<Equipment[]>> {
    this.logger.log('📚 GET /api/equipments/available → equipments-service [equipment.find.available]');
    
    try {
      const response = await firstValueFrom(
        this.equipmentsClient.send<ServiceResponse<Equipment[]>>('equipment.find.available', {}).pipe(
          timeout(5000),
          catchError(() => {
            throw new HttpException(
              'equipments-service no disponible',
      ...
      this.logger.log(`📚 Respuesta: ${response.data?.length || 0} equipos disponibles`);
      return response;
    } catch (error) {
      if (error instanceof HttpException) throw error;
      const message = error instanceof Error ? error.message : 'Error interno';
  ...
  async findOne(@Param('id') id: string): Promise<ServiceResponse<Equipment>> {
    this.logger.log(`📚 GET /api/equipments/${id} → equipments-service [equipment.find.one]`);
    
    try {
      const response = await firstValueFrom(
        this.equipmentsClient.send<ServiceResponse<Equipment>>('equipment.find.one', { id }).pipe(
          timeout(5000),
          catchError(() => {
            throw new HttpException(
              'equipments-service no disponible',
   ...
   */
  @Post()
  async create(@Body() createEquipmentDto: CreateEquipmentDto): Promise<ServiceResponse<Equipment>> {
    this.logger.log(`📚 POST /api/equipments → equipments-service [equipment.create]`);
    this.logger.log(`📚 Equipo: ${createEquipmentDto.name}`);
    
    try {
      const response = await firstValueFrom(
        this.equipmentsClient.send<ServiceResponse<Equipment>>('equipment.create', createEquipmentDto).pipe(
          timeout(5000),
          catchError(() => {
            throw new HttpException(
              'equipments-service no disponible',
  ...
  async getEquipmentWithOrders(@Param('id') id: string): Promise<ServiceResponse<EquipmentWithOrders>> {
    this.logger.log(`🔍 GET /api/equipments/${id}/repair-orders → Combinando datos de ambos servicios`);
    ...
    try {
      ...
      if (!ordersResponse.success) {
        throw new HttpException('Error al obtener órdenes', HttpStatus.INTERNAL_SERVER_ERROR);
      }
      ...
      this.logger.log(`✅ Equipo encontrado con ${equipmentOrders.length} órdenes de reparación`);
