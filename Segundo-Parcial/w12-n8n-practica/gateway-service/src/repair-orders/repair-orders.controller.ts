...
export class RepairOrdersController {
  ...
  /**
   * PATCH /api/repair-orders/:id/reject - Rechazar reparación
   */
  @Patch(':id/reject')
  async rejectOrder(
    @Param('id') id: string,
    ...
    this.logger.log(`📖 PATCH /api/repair-orders/${id}/reject → repair-orders-service [repair_order.reject]`);
    this.logger.log(`📖 Razón de rechazo: ${body.failureReason}`);
    
    try {
      const response = await firstValueFrom(
        this.repairOrdersClient.send<ServiceResponse<RepairOrder>>('repair_order.reject', { 
          id, 
      ...
      if (!response.success) {
        throw new HttpException(response.error || 'Error rechazando orden', HttpStatus.BAD_REQUEST);
      }

      this.logger.log(`✅ Orden rechazada correctamente`);
      return response;
    } catch (error) {
      if (error instanceof HttpException) throw error;
      const message = error instanceof Error ? error.message : 'Error interno';
